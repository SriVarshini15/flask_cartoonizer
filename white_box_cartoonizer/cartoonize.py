...
class WB_Cartoonize:
    ...
    def process_video(self, fname, frame_rate):
        ...
        output_fname = os.path.abspath('{}/{}-{}.mp4'.format(
            fname.replace(os.path.basename(fname), ''), str(uuid.uuid4())[:7], os.path.basename(fname).split('.')[0]))

        out = skvideo.io.FFmpegWriter(output_fname, inputdict={'-r': str(frame_rate)},
        ...
        while True:
            ...
            # Optional: Resize frame before processing (speed boost)
            frame = cv2.resize(frame, (480, 270))  # Reduce resolution for speed

            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            cartoon = self.infer(frame)
            cartoon = cv2.resize(cartoon, target_size)

            out.writeFrame(cartoon)
            count += 1

            # Optional: Print progress
            print(f"Processing frame {count}/{total_frames}", end='\r')
        ...
        p = subprocess.Popen(['ffmpeg', '-i', output_fname, "-pix_fmt", "yuv420p", final_name])
        p.communicate()
        p.wait()

        os.system("rm " + output_fname)

        print(f"\nFinished processing. Output saved to {final_name}")
        return final_name
